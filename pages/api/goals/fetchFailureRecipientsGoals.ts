...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: goals, error } = await supabaseServer
      ...
      .eq('failure_recipient_address', lowercasedUserWalletAddress) // Filter by the creator's address
    ...
    return res.status(200).json({ message: 'Goals fetched successfully', goals: goals as Goal[] });
